...
constexpr uint32_t kGopFrameCount = 16;
constexpr uint32_t kMaxDpbSlots = 16;
constexpr uint32_t kMaxActiveReferenceSlots = 16;
constexpr uint32_t kPictureGranularity = 16;
...
constexpr StdVideoH264ProfileIdc kProfileIdc = STD_VIDEO_H264_PROFILE_IDC_MAIN;
constexpr StdVideoH264LevelIdc kLevelIdc = STD_VIDEO_H264_LEVEL_IDC_4_1;
constexpr vk::VideoChromaSubsamplingFlagsKHR kChromaSubsampling = vk::VideoChromaSubsamplingFlagBitsKHR::e420;
constexpr StdVideoH264ChromaFormatIdc kChromaFormatIdc = STD_VIDEO_H264_CHROMA_FORMAT_IDC_420;
constexpr vk::Format kDpbImageFormat = vk::Format::eG8B8R82Plane420Unorm;
constexpr vk::Format kEncodeInputImageFormat = vk::Format::eG8B8R82Plane420Unorm;
constexpr vk::Format kRgbFormat = vk::Format::eR8G8B8A8Unorm;
constexpr vk::Format kYPlaneFormat = vk::Format::eR8Unorm;
constexpr vk::Format kUvPlaneFormat = vk::Format::eR8G8Unorm;
}  // namespace

H264Encoder::H264Encoder(const vvk::ExecutionContext& execution_context, VkDevice device, uint32_t video_queue_index,
...
void H264Encoder::WriteEncodeCommands(VkCommandBuffer command_buffer, VkImage image) {
...
std::string H264Encoder::GetEncodedData(VkImage image) {
  std::vector<vk::ImageMemoryBarrier2> image_barriers;
  uint32_t encodable_image_index = std::numeric_limits<uint32_t>::max();
  for (uint32_t i = 0; i < encodable_images_.size(); i++) {
    if (encodable_images_[i] == image) {
...
std::string H264Encoder::GetEncodedHeader() const {
...
void H264Encoder::CreateQueryPool() {
...
void H264Encoder::InitializeVideoProfile() {
...
void H264Encoder::AllocateCommandBuffer() {
...
void H264Encoder::InitializeVideoSession() {
...
void H264Encoder::BindVideoSessionMemory() {
  uint32_t video_session_memory_requirements_count = 0;
  dev_dispatch_.GetVideoSessionMemoryRequirementsKHR(device_, video_session_, &video_session_memory_requirements_count,
                                                     nullptr);
  ...
  dev_dispatch_.GetVideoSessionMemoryRequirementsKHR(device_, video_session_, &video_session_memory_requirements_count,
                                                     video_session_memory_requirements.data());
...
void H264Encoder::AllocateDpbImages() {
...
void H264Encoder::InitializeVideoSessionParameters() {
  std_sps_vui_flags_ = {
...
void H264Encoder::InitializeRateControl() {
...
void H264Encoder::AllocateOutputBuffer() {
...
void H264Encoder::AllocateInputImage() {
  VmaAllocationCreateInfo allocation_create_info = {};
  VkResult result =
...
void H264Encoder::AllocateIntermediaryYuvImage() {
  VmaAllocationCreateInfo allocation_create_info = {};
...
void H264Encoder::CreateIntermediaryRgbImage() {
  VmaAllocationCreateInfo allocation_create_info = {};
  VkResult result =
      vmaCreateImage(execution_context_.allocator(),
                     vk::ImageCreateInfo{
...
void H264Encoder::CreateRgbToYuvComputePipeline() {
...
void H264Encoder::AllocateRgbToYuvDescriptorSets() {
  ...
  dev_dispatch_.UpdateDescriptorSets(device_, static_cast<uint32_t>(descriptor_writes.size()), descriptor_writes.data(),
                                     0, nullptr);
}
...
void H264Encoder::InitializePictureParameters() {
...
void H264Encoder::UpdatePictureParameters(uint32_t picture_index) {
